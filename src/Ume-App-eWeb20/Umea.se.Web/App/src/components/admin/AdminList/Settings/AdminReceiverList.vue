<template>
	<div class="admin-receiver-list">
		<div class="search-wrap admin-list-search-wrap mt-5">
			<ume-text-field
				:placeholder="$t('component.adminReceiverManager.search')"
		...
		<BaseContextMenu ref="contextMenu">
			<v-list v-if="clickedId" class="admin-list-context-menu">
				...
				>
					<v-list-item-title>
						{{ $t('component.adminReceiverManager.delete') }}
					</v-list-item-title>
				</v-list-item>
			</v-list>
		</BaseContextMenu>
		...
		<!-- No results -->
		<v-alert
			v-else-if="!receiverItems.length"
		...
		>
			...
			<template v-slot:bottom>
				<admin-list-pagination
					v-model:items-per-page="itemsPerPage"
					v-model:page="page"
					:number-of-items="receiverItems.length"
				/>
			</template>
		</v-data-table>

		<admin-receiver-edit
			:receiver="editingItem"
...
import { IReceiver, IRootState, ISortBy, ITableHeader } from '@/models/IForm';
...
import AdminReceiverEdit from './AdminReceiverEdit.vue';
...
const receiverItems = computed(() => {
	if (store.state.receivers !== null) {
		let items = store.state.receivers;
		if (search.value) {
...
const editingItem = ref<IReceiver | null>(null);
...
const onEdit = (id: number): void => {
	const item = receiverItems.value.find((item) => item.id === id);
	editingItem.value = item ?? null;
};

const onSave = async (receiver: IReceiver): Promise<void> => {
	try {
		if (Math.sign(receiver.id) === -1) {
			// Create new receiver
			await store.dispatch('createReceiver', {
				receiver: {
					...receiver,
					id: null,
				},
		...
		if (err === ErrorCode.MissingTitle) {
			ErrorService.onError({
				err: new Error('saveReceiver is missingtitle!'),
				message: t(
					'component.adminReceiverManager.error.onSaveReceiver.missingTitle'
				),
				log: false,
			});
		} else if (err === ErrorCode.MissingUrl) {
			ErrorService.onError({
				err: new Error('saveReceiver is missing path!'),
				message: t(
					'component.adminReceiverManager.error.onSaveReceiver.missingPath'
				),
				log: false,
			});
		} else {
...
async function deleteItem(id: number): Promise<void> {
	...
	const doDeleteItem = await tConfirmDialogAsync(
		...
		{
			text: t('component.adminReceiverManager.delete'),
			color: 'error',
		}
	);
	if (doDeleteItem) {
		try {
			await store.dispatch('deleteReceiver', {
				receiver: item,
			});
		} catch (err) {
			if (err === ErrorCode.CannotDeleteObjectWithRelation) {
				ErrorService.onError({
					err: new Error('deleteReceiver have a relation to !'),
					message: t(
						'component.adminReceiverManager.error.onDeleteReceiver.restricted'
					),
					log: false,
				});
			} else {
...
onMounted(async () => {
	await store.dispatch('getReceiverList');
