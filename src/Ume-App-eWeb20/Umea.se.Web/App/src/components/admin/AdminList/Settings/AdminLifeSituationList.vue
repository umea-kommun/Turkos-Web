<template>
	<div class="admin-life-situation-list">
		<div class="search-wrap admin-list-search-wrap mt-5">
			<ume-text-field
				:placeholder="$t('component.adminLifeSituationManager.search')"
			...
			>
				...
				{{ $t('component.adminLifeSituationManager.create') }}
			</v-btn>
		</div>
		...
		<BaseContextMenu ref="contextMenu">
			<v-list v-if="clickedId" class="admin-list-context-menu">
				...
				>
					<v-list-item-title>
						{{ $t('component.adminLifeSituationManager.delete') }}
					</v-list-item-title>
				</v-list-item>
			</v-list>
		</BaseContextMenu>
		...
		<!-- No results -->
		<v-alert
			v-else-if="!lifeSituationItems.length"
		...
		>
			...
			<template v-slot:bottom>
				<admin-list-pagination
					v-model:items-per-page="itemsPerPage"
					v-model:page="page"
					:number-of-items="lifeSituationItems.length"
				/>
			</template>
		</v-data-table>

		<admin-life-situation-edit
			:lifeSituation="editingItem"
...
import AdminLifeSituationEdit from './AdminLifeSituationEdit.vue';
...
const lifeSituationItems = computed(() => {
	if (store.state.lifeSituations !== null) {
		let items = store.state.lifeSituations;
		if (search.value) {
...
const editingItem = ref<ILifeSituation | null>(null);
...
const onEdit = (id: number): void => {
	const item = lifeSituationItems.value.find((item) => item.id === id);
	editingItem.value = item ?? null;
};

const onSave = async (lifeSituation: ILifeSituation): Promise<void> => {
	try {
		if (Math.sign(lifeSituation.id) === -1) {
			// Create new life situation
			await store.dispatch('createLifeSituation', {
				lifeSituations: {
					...lifeSituation,
					id: null,
				},
			...
			// Update existing life situation
			await store.dispatch('updateLifeSituation', {
		...
		if (err === ErrorCode.MissingTitle) {
			ErrorService.onError({
				err: new Error('createLifeSituation is missingtitle!'),
				message: t(
					'component.adminLifeSituationManager.error.onSaveLifeSituation.missingTitle'
				),
				log: false,
			});
		} else {
...
async function deleteItem(id: number): Promise<void> {
	...
	const doDeleteItem = await tConfirmDialogAsync(
		t('component.adminLifeSituationManager.confirmDelete.title'),
		t('component.adminLifeSituationManager.confirmDelete.text', [
			item?.title,
		]),
		{
			text: t('component.adminLifeSituationManager.delete'),
			color: 'error',
		}
	);
	if (doDeleteItem && item) {
		try {
			await store.dispatch('deleteLifeSituation', {
				lifeSituations: item,
			});
		} catch (err) {
			if (err === ErrorCode.CannotDeleteObjectWithRelation) {
				ErrorService.onError({
					err: new Error('deleteLifeSituation have a relation to !'),
					message: t(
						'component.adminLifeSituationManager.error.onDeleteLifeSituation.restricted'
					),
					log: false,
				});
			} else {
...
onMounted(async () => {
	await store.dispatch('getLifeSituationList');
